...
test_that("Creating and characteristics 1", {
  ...
  y <- data.frame(first=rep(-1.23, 10), second=rep("A", 10), stringsAsFactors=FALSE)
  expect_that(x[stringsAsFactors=FALSE], equals(y))
  expect_that(x[,1:2,stringsAsFactors=FALSE], equals(y[,1:2]))
  expect_that(x[,1], equals(y[,1]))
  expect_that(x[,1,drop=FALSE], equals(y[,1,drop=FALSE]))
  expect_that(x[,2,stringsAsFactors=FALSE], equals(y[,2]))
  expect_that(x[,2,drop=FALSE,stringsAsFactors=FALSE], equals(y[,2,drop=FALSE]))
})
